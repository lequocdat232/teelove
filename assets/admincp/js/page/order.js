$(function(){
    ...
    function renderTableRow(data) {
        ...
        switch (data.shipping_status) {
            case '0':
                shipping_status = '<span class="badge badge-pill badge-danger">Not Ship</span>'
                break;
            case '1':
                shipping_status = '<span class="badge badge-pill badge-info">Sending</span>'
                break;
            case '2':
                shipping_status = '<span class="badge badge-pill badge-primary">PP Send</span>'
                break;
            case '3':
                shipping_status = '<span class="badge badge-pill badge-success">Shipped</span>'
                break;
            default:
                // code...
                break;
        ...
        switch (data.status) {
            case '0':
                status_order = '<span class="badge badge-pill badge-primary">New</span>'
                break;
            case '1':
                status_order = '<span class="badge badge-pill badge-secondary">Processing</span>'
                break;
            case '2':
                status_order = '<span class="badge badge-pill badge-warning">Hold</span>'
                break;
            case '3':
                status_order = '<span class="badge badge-pill badge-danger">Cancel</span>'
                break;
            case '4':
                status_order = '<span class="badge badge-pill badge-success">Complete</span>'
                break;
            default:
                // code...
                break;
        ...
        row += '<tr>' 
            ...
            + '<td>'
                + '<button type="button" class="btn cur-p btn-info btn-view-detail" data-id="'+data.id+'">'
                    + '<i class="ti-eye"></i>'
                + '</button> '
    ...
    function getList() {
        ...
        $.ajax({
            url: "order/get-range" + window.location.search ,
            dataType: 'JSON',
        })
        .done(function(data) {
    ...
    $(`#form_search_order`).submit(function(e){
        e.preventDefault()
        $('#searchButton').click()
    })
    ...
    $(document).on('click', '.btn-view-detail', function() {
        ...
        // console.log(id);
        $('#name_table').html('Order Sub Table')
        $.ajax({
            url: 'order/view-detail-order/' + id,
            type: 'POST',
            dataType: 'JSON',
        }).done(function(response) {
            $('#order_id').empty();
            $('#order').css({display: 'none'})
            $('#order-sub').css({display: 'block'})
            $('#order-sub tbody').html(renderTableDetail(response));
        }).fail(function(error) {
    ...
    function renderTableDetail(data) {
        ...
        $('#order_id').append(data[0].order_id);
        return html;
    }
    ...
    function renderRowDetail(data) {
        ...
        row += '<tr>' 
            + '<td>'+ data.id +'</td>'
            + '<td>'+ data.lot_name +'</td>'
            + '<td><img src=" ' + data.url + '"alt="'+ data.lot_name +'" height="70"></td>'
    ...
    $(document).on('click', '#btn-back', function () {
        $('#name_table').html('Order Table')
        $('#order').css({display: 'block'})
        $('#order-sub').css({display: 'none'})
    })
