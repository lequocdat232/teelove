(function($) {
    ...
    function initCart() {
            ...
            .done(function(response) {
                ...
                if (!$('#user_id').val()) {
                    ...
                    $.ajax({
                        ...
                        type: 'post',
                        dataType: 'json',
                        data: {cart: cart, user_id: $('#user_id').val()},
                    })
                    .done(function(response) {
                        // console.log("success", response);
                        updateCartHeader(JSON.parse(response.cart_data))
        ...
        //     var cart = window.localStorage.cart
        //     if (cart) {
        //         // console.log('call request update cart to server');
        //         cart = JSON.parse(cart)
        //         cart = cart.filter(function(val) {
        //             return val != null
        //         });
    ...
    function initWishList() {
        if (!$('#user_id').val()) {
            // console.log(window.localStorage.cart);
            var wishList = window.localStorage.wishList
            if (wishList) {
            ...
            if (wishList) {
                ...
                $.ajax({
                    ...
                    type: 'post',
                    dataType: 'json',
                    data: {wishList: wishList, user_id: $('#user_id').val()},
                })
                .done(function(response) {
                    console.log("success", response);
                    // updateCartHeader(JSON.parse(response.cart_data))
...
function renderAttr(attr) {
    var string = ''
    attr.map(function (val) {
        string += `<li>${val.name_text}: ${val.attr_value}</li>`
    })
    return string;
}
...
function updateCartHeader(cart) {
    ...
    $.each(cart, function(index, val) {
        if (val) {
            totalQty += +val.qty
            totalMoney += +val.qty * +val.product.price
            dropcart.append(`
                <div class="dropcart__products-list">
                    <div class="dropcart__product">
                        <div class="dropcart__product-image">
                            <a href="product.html"><img src="${val.product.url}" alt=""></a>
                        </div>
                        <div class="dropcart__product-info">
                            <div class="dropcart__product-name"><a href="product.html">${val.product.lot_name}</a></div>
                            <ul class="dropcart__product-options">
                                ${renderAttr(val.product.attr)}
                            </ul>
                            <div class="dropcart__product-meta">
                                <span class="dropcart__product-quantity">${val.qty}</span> ×
                                <span class="dropcart__product-price">$${new Number(val.product.price).toLocaleString()}</span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    $('#indicator__value').text(totalQty)
    $('#indicator__value--mobi').text(totalQty)
    $('.dropcart #header-cart-total').text('$' + new Number(totalMoney).toLocaleString())
}
